...
量子コンピュータは特定の計算タスクを従来型のコンピュータよりも高速に実行できることが知られています。
このグループでは、量子コンピュータの計算能力を使ってどのようなことができそうか、また量子コンピュータの力をさらに引き出すにはどうすればよいかを研究しています。

## 学生の受け入れについて
- 本グループでは現状各年度 2 名程度を受け入れられます（年によって増減します）。
- グループの規模はおおむね 10 名程度です（年度により変動します）。
...
## 研究環境・方針について
- 本グループの研究は主に計算機上で進めます。いわゆる大型の実験装置を扱う研究室ではありません。
- 博士課程学生・博士課程進学希望者には、RA 制度などを通して可能な限り経済的に支援します。
- RA は研究を仕事として行ってもらう制度で、時給と稼働時間に応じて給与を支給します。博士課程学生は原則として RA で支援します。修士課程でも状況に応じて RA 支援を行うことがあります。
- [大阪大学量子情報・量子生命研究センター (QIQB)](https://qiqb.osaka-u.ac.jp/) と緊密に連携します。
  - QIQB の超伝導量子コンピュータも利用可能です。
  - 必要に応じて QIQB の教員や研究員とも連携し、専門的な相談や技術的なサポートを受けられる体制を整えています。
- 配属された学生には PC を貸与します。
- Slack, Overleaf, github などを使って情報共有を行っています。研究室内には可能な限り情報をオープンにします。
- コアタイムなどはありませんが、特に配属当初は大学に来て研究することを推奨します。
- 研究の進め方は柔軟です。普段の作業は学内外どちらでも進められますが、週 1 回程度の対面ミーティングを基本として、研究の相談や共有を行っています。
- SSH サーバーを立てているので、学外からも学内と同じように研究することが可能です。計算機等はすべてリモートアクセスできる環境を整えています。
- [Nielsen-Chuang の教科書](https://www.cambridge.org/highereducation/books/quantum-computation-and-quantum-information/01E10196D0A682A6AEFFEA52D53BE9AE#overview) の 1, 2, 4 章は最低限の知識として必要です。このグループに興味がある方は、一度読んでみることをおすすめします。
...
量子コンピュータが最も得意とするのは、量子力学に従う物理系のシミュレーションです。
従来型のコンピュータで量子的な重ね合わせ状態をシミュレートするには、一般に物理系のサイズに対して指数的な計算量が必要となってしまいますが、量子コンピュータであれば多項式の計算量でシミュレートできることが知られています。
特に分子や物質の電子状態をシミュレートする量子化学計算（第一原理計算）は、産業的にも良く利用されている計算タスクであり、量子コンピュータの一番の応用先です。
...
機械学習は目覚ましい成果を挙げながら社会実装が進んでいます。
この機械学習という分野に量子コンピュータの計算能力を持ち込み、さらに高度化することはできるでしょうか？
...
量子誤り訂正はノイズをほぼ完全に取り除くことができる技術ですが、その実現には多大なオーバーヘッドが必要で、例えば量子化学計算で実用的な計算をするには 100 万量子ビット程度の集積化が必要とされています。
現実的には 100 万量子ビットに到達して、理想的な誤り耐性量子コンピュータ (FTQC) を実現する途上で、ある程度ノイズが取り除かれているがすべては取り除けていないようなデバイス (early-FTQC) が出現することが予想されます。
